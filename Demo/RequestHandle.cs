    ...
    public class RequestHandle
    {
        ...
        private string[] GetRenamedImages(int picIndex,string imgs, string prefix)
        {
           ...
           // Regex maxIdRegex=new Regex(prefix+"(\\d+)\\.");
            ...
            {
                picIndex++;
                imgPrefix = String.Concat(
                    regex.Replace(img, prefix),
                    picIndex > 10 ? index.ToString() : "0" + picIndex.ToString()
                    );

                img2 = String.Concat(imgPrefix, ".jpg");

                if (img.IndexOf(imgPrefix) != -1)
                {
                    reImgArr[index] = img;
                }
                else
...
IF NOT EXISTS (SELECT cashNo FROM cir_case WHERE cashNo=@cashNo)
INSERT INTO [cir_case]
		([CashNo]
        ,[caseNo]
        ,[cusCaseNo]
		,[partnerCode]
