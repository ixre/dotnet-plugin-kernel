...
namespace com.mapfre.poi
{
    internal class Helper
    {
        ...
        public static bool IsWapPortal(HttpRequest req)
        {
            if (Config.DebugMode)
            {
                return true;
        ...
        public static DataBaseAccess DBA
        {
            get
            {
                if (connString == null)
                {
                    connString = Config.PluginAttrs.Settings["dbconn"];
                    string dir = PluginUtil.GetAttribute<Main>().WorkSpace;
                    connString = connString.Replace("$ROOT", dir.Substring(0, dir.Length - 1));
                }
