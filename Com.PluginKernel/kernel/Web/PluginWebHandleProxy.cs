...
namespace Com.PluginKernel.Web
{
    public class PluginWebHandleProxy<T> : PluginHandleProxy<T>
    {
        ...
        // 此段代码仅供参考
        //
        bool HanleRequestUse<HandleClass>(HandleClass t, T context, bool isPostRequest, string requestPath)
        {
            string action = null;
            ...
            {
                action = requestPath.IndexOf('/') == -1 ? requestPath : requestPath.Substring(0, requestPath.IndexOf('/'));
            }
            ...
            if (method != null)
            {
                ...
                {
                    LogHandler.RecordException(httpContext.Request.RawUrl,exc);
                }
