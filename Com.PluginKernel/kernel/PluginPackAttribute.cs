...
namespace Com.PluginKernel
{
    ...
    public class PluginInfoAttribute : Attribute
    {
        ...
        public string ID
        {
            get
            {
                if (id == null)
                {
                    id = String.Format("{0}{1}", GetType().Assembly.GetName().Name, IndexNum == 0 ? string.Empty : "_" + IndexNum);
                }
                return id;
            }
        }
    ...
    public class PluginPackAttribute : Attribute
    {
        ...
        public PluginState State
        {
            get
            {
                return (settings["state"] == "1" || settings["state"] == "Normal") ? PluginState.Normal : PluginState.Stop;
            }
            set
            {
